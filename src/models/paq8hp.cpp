...
public:
  ...
  void add(int x, bool add_export = true) {
    if (add_export) AddPrediction(squash(x));
    tx[nx++]=x;
  }
...
inline int mix2(Mixer& m, int s, StateMap& sm) {
  if (s == 0) {
    if (cxtfl) {
      m.add(0); m.add(0); m.add(0, false); m.add(0, false);
    } else {
 ...
 if (cxtfl) {
  ...
  m.add((p1-p0)*3/64);
  m.add(st*(n1-n0)*3/16);
  m.add(((p1&n0)-(p0&n1)) /16, false);
  m.add(((p0&n0)-(p1&n1))*7/64, false);
  return s>0;
 }
  m.add(st*9/32);
  m.add(st*(n1-n0)*3/16);
