...
float Predictor::Predict() {
  ...
  const auto& paq_model_outputs = paq_model_->Predict();
  for (unsigned int j = 0; j < paq_model_outputs.size(); ++j) {
    layers_[0].SetInput(input_index, paq_model_outputs[j]);
    ++input_index;
  }
  auto paq_model_index = input_index - 1;
 ...
 for (unsigned int i = 0; i < indirect_r_models_.size(); ++i) {
  ...
  float auxiliary_average = Sigmoid::Logistic(layers_[0].Inputs()[paq_model_index]) + Sigmoid::Logistic(layers_[0].Inputs()[byte_mixer_index]);
  ...
  layers_[1].SetStretchedInput(mixer_0_.size(), layers_[0].Inputs()[paq_model_index]);
